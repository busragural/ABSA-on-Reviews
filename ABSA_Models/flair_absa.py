...
def get_sentiment_score_flair(text):
    """Flair ile duygu analizi yapar ve skor döndürür."""
    sentence = Sentence(text)
    classifier.predict(sentence)  # Model tahmin yapar
    
    label = sentence.labels[0]  # En güçlü tahmin
    value = float(label.score)  # Tahminin güven skoru

    if label.value == "NEGATIVE":
        return -value  # Negatifse - olarak al
    else:
        return value  # Pozitifse olduğu gibi al
    ...
    for sentence in doc.sents:  # Cümlelere ayır
        ...
        for aspect, keywords in aspects.items():
            if any(keyword.lower() in sentence_text_lower for keyword in keywords):
                score = get_sentiment_score_flair(sentence.text)  # Flair ile sentiment analizi yap
...
output_file = "./Data/output/Flair_ABSA.csv"
